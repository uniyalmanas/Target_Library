...
export function MemberProfileContent({
  ...
  const getRenewUrl = () => {
    if (receipts.length === 0) {
      return `/l/${encodeURIComponent(slug)}/new-receipt?student_id=${member.student_id}`;
    }
    ...
    return `/l/${encodeURIComponent(slug)}/new-receipt?${searchObj.toString()}`;
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between pb-3 border-b border-panel-border flex-wrap gap-2">
        <div className="flex items-center gap-3">
          ...
          <Link
            href={`/l/${encodeURIComponent(slug)}/members`}
...
export default function MemberProfileView({
  ...
  return (
    <Suspense fallback={<p className="text-neutral-400 text-center py-10">Loading member profile...</p>}>
      <MemberProfileContent tenantSlug={tenantSlug} memberId={memberId} />
    </Suspense>
  );
}
