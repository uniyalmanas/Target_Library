              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5 max-h-[350px] overflow-y-auto p-1">
                  {doubleShiftSeatsList.map((s) => (
                    <div
                      key={s.seat_id}
                      className="bg-purple-50/50 hover:bg-purple-100/60 dark:bg-card-bg dark:hover:bg-neutral-800/60 border border-purple-300 dark:border-panel-border p-4 rounded-2xl flex flex-col justify-between gap-3 shadow-2xs transition"
                    >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5 max-h-[350px] overflow-y-auto p-1">
                  {fullSeatsList.map((s) => (
                    <div
                      key={s.seat_id}
                      className="bg-rose-50/50 hover:bg-rose-100/60 dark:bg-card-bg dark:hover:bg-neutral-800/60 border border-rose-300 dark:border-panel-border p-4 rounded-2xl flex flex-col justify-between gap-3.5 shadow-2xs"
                    >
                      <div className="flex justify-between items-center">
                        <p className="text-base font-extrabold text-foreground flex items-center gap-1.5">
                          <span className="text-sm">🪑</span> Seat {s.seat_number}
                        </p>
                        <span className="text-[9px] font-black uppercase bg-rose-100 text-rose-950 dark:bg-rose-500/15 dark:text-rose-400 border border-rose-400 dark:border-rose-500/25 px-2.5 py-1 rounded-full tracking-wider">
                          Full Day
                        </span>
                      </div>
                      <div className="space-y-2">
                        {s.receipts.map((r: any) => (
                          <Link
                            key={r.receipt_no}
                            href={`/l/${encodeURIComponent(slug)}/members/${r.student_id}`}
                            className="block text-xs p-3 rounded-xl bg-card-bg hover:bg-rose-100/40 dark:bg-background/60 dark:hover:bg-rose-500/10 border border-panel-border hover:border-rose-400 dark:hover:border-rose-500/25 transition group cursor-pointer shadow-2xs"
                          >
                            <div className="flex justify-between">
                              <span className="font-bold text-foreground group-hover:text-rose-500 dark:group-hover:text-rose-400 transition-colors">
                                {r.member?.name}
                              </span>
                              <span className="text-[10px] text-text-muted font-mono">
                                ID: #{r.student_id}
