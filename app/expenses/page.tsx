...
function ExpensesContent() {
  ...
  useEffect(() => {
    fetchExpenses();
  }, [slug, selectedMonth, selectedCategory]);
    ...
    <div className="w-full max-w-[96vw] 2xl:max-w-[1750px] mx-auto px-4 md:px-8 py-6 space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-2 flex-1 max-w-md">
          <input
            ...
            className="w-full px-3.5 py-2 rounded-xl bg-card-bg border border-panel-border text-xs text-foreground placeholder:text-text-muted focus:outline-none focus:border-rose-500"
          />
        </div>
      ...
      <div className="bg-panel-bg border border-panel-border rounded-2xl overflow-hidden shadow-xs">
        {loading ? (
          ...
          <div className="p-12 text-center text-xs text-text-muted space-y-3">
            <p className="text-base font-bold text-foreground">No expenses recorded for this period.</p>
            <p>Click "+ Record Expense" above to add your electricity, rent, or maintenance costs.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-neutral-500/5 border-b border-panel-border text-text-muted text-[10px] font-black uppercase tracking-wider">
              ...
              <tbody className="divide-y divide-panel-border/50">
                {filteredExpenses.map((exp) => {
                  ...
                  return (
                    <tr key={exp.id} className="hover:bg-neutral-500/5 transition">
                      <td className="py-3.5 px-4 font-mono whitespace-nowrap text-text-muted">
                        {exp.expense_date}
                      </td>

                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-neutral-500/10 border border-panel-border text-text-main">
                          <span>{meta.icon}</span>
                          <span>{meta.label}</span>
                        </span>
                      </td>

                      <td className="py-3.5 px-4 font-bold text-foreground max-w-xs">
                        {exp.title}
                      </td>
                      ...
                      <td className="py-3.5 px-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-extrabold uppercase ${
                            exp.payment_mode === "online" || exp.payment_mode === "upi"
                      ...
                      <td className="py-3.5 px-4 text-right font-mono font-black text-sm text-foreground whitespace-nowrap">
                        ₹{Number(exp.amount).toLocaleString("en-IN")}
                      </td>

                      <td className="py-3.5 px-4 text-text-muted text-[11px] max-w-sm truncate">
                        {exp.notes || <span className="italic text-neutral-400">—</span>}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right whitespace-nowrap">
                        <button
                          ...
                          className="px-2.5 py-1 rounded-lg text-neutral-400 hover:text-rose-600 hover:bg-rose-500/10 transition text-xs font-bold cursor-pointer disabled:opacity-50"
